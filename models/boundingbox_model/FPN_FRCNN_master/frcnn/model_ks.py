...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # FRCNN_class_logits = KL.Dropout(0.40)(FRCNN_class_logits)                                            
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="FRCNN_mask_conv1")(x)
    x = KL.TimeDistributed(BatchNorm(),
                           name='FRCNN_mask_bn1')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="FRCNN_mask_conv2")(x)
    x = KL.TimeDistributed(BatchNorm(),
                           name='FRCNN_mask_bn2')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="FRCNN_mask_conv3")(x)
    x = KL.TimeDistributed(BatchNorm(),
                           name='FRCNN_mask_bn3')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="FRCNN_mask_conv4")(x)
    x = KL.TimeDistributed(BatchNorm(),
                           name='FRCNN_mask_bn4')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="FRCNN_mask_deconv")(x)
...
def FRCNN_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    def __getitem__(self, idx):
        ...
        while b < self.batch_size:
            ...
            if self.random_rois:
                ...
                if self.detection_targets:
                    rois, frcnn_class_ids, frcnn_bbox = \
                        build_detection_targets(
                            rpn_rois, gt_class_ids, gt_boxes, self.config)
                ...
                if self.random_rois:
                    ...
                    if self.detection_targets:
                        ...
                        batch_frcnn_class_ids = np.zeros(
                            (self.batch_size,) + frcnn_class_ids.shape, dtype=frcnn_class_ids.dtype)
                        batch_frcnn_bbox = np.zeros(
                            (self.batch_size,) + frcnn_bbox.shape, dtype=frcnn_bbox.dtype)
            ...
            if self.random_rois:
                ...
                if self.detection_targets:
                    batch_rois[b] = rois
                    batch_frcnn_class_ids[b] = frcnn_class_ids
                    batch_frcnn_bbox[b] = frcnn_bbox
        ...
        if self.random_rois:
            ...
            if self.detection_targets:
                ...
                batch_frcnn_class_ids = np.expand_dims(
                    batch_frcnn_class_ids, -1)
                outputs.extend(
                    [batch_frcnn_class_ids, batch_frcnn_bbox])
...
class FasterRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            FRCNN_class_logits, FRCNN_class, FRCNN_bbox =\
                fpn_classifier_graph(rois, FRCNN_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)

            ####################################################################################
            # FRCNN_mask = build_fpn_mask_graph(rois, FRCNN_feature_maps,
            #                                   input_image_meta,
            ...
            ####################################################################################
            # mask_loss = KL.Lambda(lambda x: FRCNN_mask_loss_graph(*x), name="FRCNN_mask_loss")(
            #     [target_mask, target_class_ids, FRCNN_mask])
            ####################################################################################
            ...
            #            FRCNN_class_logits, FRCNN_class, FRCNN_bbox, FRCNN_mask,
            ...
            FRCNN_class_logits, FRCNN_class, FRCNN_bbox =\
                fpn_classifier_graph(rpn_rois, FRCNN_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
            ####################################################################################
            ...
            # FRCNN_mask = build_fpn_mask_graph(detection_boxes, FRCNN_feature_maps,
            #                                   input_image_meta,
            #                                   config.MASK_POOL_SIZE,
            #                                   config.NUM_CLASSES,
            #                                   train_bn=config.TRAIN_BN)
            ...
            #                      FRCNN_mask, rpn_rois, rpn_class, rpn_bbox],
            #                  name='FASTER_RCNN')
        ...
        if config.GPU_COUNT > 1:
            from FRCNN.parallel_model import ParallelModel
    ...
    def compile(self, learning_rate, momentum):
        ...
        #     "FRCNN_class_loss", "FRCNN_bbox_loss", "FRCNN_mask_loss"]
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # Data generators
        train_generator = DataGenerator(train_dataset, self.config, shuffle=True, augment=False,
    ...
    def unmold_detections(self, detections, original_image_shape,
        ...
        FRCNN_mask: [N, height, width, num_classes]
        ...
        # masks = FRCNN_mask[np.arange(N), :, :, class_ids]
    ...
    def detect_molded(self, molded_images, image_metas, verbose=0):
        ...
        for i, image in enumerate(molded_images):
            ...
            final_rois, final_class_ids, final_scores, final_masks =\
                self.unmold_detections(detections[i], FRCNN_mask[i],
                                       image.shape, molded_images[i].shape,
                                       window)
            results.append({
                "rois": final_rois,
